...
每台设备检查：全新安装、权限/通知、首次启动、沉浸全屏、无资源首屏、二维码、上传后自动播放、暂停/继续状态、无进度和多余控制、Activity 重建、Service 存活、开机恢复。
...
- [ ] Android 正常重启和 `BOOT_COMPLETED` 后恢复。
...
以下任一情况必须阻断发布：签名不符、鉴权绕过、崩溃/ANR、无法开机恢复、已有资源损坏、24 小时测试失败、目标电视盒子无法全屏稳定播放、Token/路径出现在诊断导出、外网可直接访问控制端口。
