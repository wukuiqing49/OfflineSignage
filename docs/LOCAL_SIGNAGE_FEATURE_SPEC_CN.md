...
WebSocket 用于实时状态，不替代 HTTP 资源 CRUD。连接建立后先发送完整 `DEVICE_STATUS` 和 `CONTROL_SESSION`，断线重连后重新发送完整状态；客户端不能只依赖事件增量恢复界面，实时通道恢复期间应使用低频 HTTP 状态刷新兜底。
...
手机与电脑控制流程、工作区职责、控制权、安全约束和后续方向见 `LOCAL_SIGNAGE_WEB_MOBILE_WORKFLOW_CN.md`。控制台必须同时支持手机扫码和电脑输入广告屏一次性配对码，并优先保证连接、当前状态、快速播放和上传路径，不能把低频设备诊断与高频播放操作混在同一长页面中。

## 11. 首次启动与显示设置

- 新安装首次人工启动显示欢迎页，说明同局域网连接、手机扫码或电脑短码配对、全屏播放三步流程。
- 用户完成欢迎页后，后续桌面启动直接进入播放页；已有内容的覆盖升级自动跳过欢迎页。
- Android 14 及以下或具备厂商自启动、Kiosk/Launcher、Device Owner 能力的专用设备，开机恢复显式进入播放页且不依赖欢迎页状态；Android 15 及以上普通设备遵循平台限制，在用户首次打开应用后恢复上次播放状态。
