...
> **实施现状（2026-09-24）**：本节为当前行为的唯一商业化事实摘要。本文后续原始方案章节是早期设计草案，未按当前实现维护，不应用于商店说明、合同或发布验收；如需继续作为实施规格，应先整体重写。

| 项目 | 当前实现/发布边界 |
| --- | --- |
| 试用 | 首次启动后完整功能 7 天；无需绑定支付方式 |
| 试用后免费模式 | 播放和现有内容保留；最多新增 10 个图片/视频资源、1 个基础图片/视频轮播；高级内容与多设备能力受限 |
| 当前新购商品 | `pro_subscription` 的年度 `pro-yearly` 自动续订订阅；购买页只展示此方案 |
| 旧购兼容 | 代码仍识别历史月度 Base Plan 与 `pro_lifetime`，仅用于兼容已有购买恢复；并非当前新购承诺 |
| 订阅离线宽限 | 最近一次成功校验后最多 7 天，以代码策略为准；无法替代 Play 服务端状态 |
| 价格 | 使用 Google Play 返回的本地化 `formattedPrice`；Play Console 实际商品、地区和价格尚未完成真实交易验收 |
| 上架门禁 | 内部测试轨道购买、取消、续订、退款、恢复及目标设备兼容性仍需实测；本地单测不代表 Play 交易已验收 |

商店/法律文案须在商品目录与价格确认、真实 Play 购买回归及法律审阅后再作为正式发布内容。下方其余旧草案仅作历史参考。
